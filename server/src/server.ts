    ...
    lintersCopy.forEach(linter => {
        try {
            ...
            if (error instanceof Error) {
                tracker.add(getErrorMessage(error, textDocument));
            } else {
                let message = new Error(String(error));
                tracker.add(getErrorMessage(message, textDocument));
            }
        }
    });
    ...
    if (activeDocument !== undefined && activeDocument !== null) {
        ...
        for (const document of documents.all()) {
            try {
                ...
                if (error instanceof Error) {
                    tracker.add(getErrorMessage(error, document));
                } else {
                    let message = new Error(String(error));
                    tracker.add(getErrorMessage(message, document));
                }
            }
        }
    }
